"""Общие хелперы админки: фильтры ссылок."""
...
def apply_link_filters(
    ...
    *,
    ...
    for pred in link_filter_predicates(platform, account, destination):
...
async def cached_platform_link_counts(db: AsyncSession) -> dict[str, int]:
    """Кэш счётчиков ссылок по платформам для сайдбара/фильтров."""
